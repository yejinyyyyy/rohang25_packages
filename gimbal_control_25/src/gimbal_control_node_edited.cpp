...
public:
    ...
    {   RCLCPP_INFO(this->get_logger(), "▶ Gimbal control node started");
        ...
        subscription_gimbal_node_control_flag1 = this->create_subscription<std_msgs::msg::Bool>( "/gimbal_node_control_flag1", qos,
                [this](const std_msgs::msg::Bool::ConstSharedPtr msg) {
                    this->gimbal_node_control_flag_1 = msg->data; }); //조난자 시작 플래그

        subscription_gimbal_node_control_flag2 = this->create_subscription<std_msgs::msg::Bool>( "/gimbal_node_control_flag2", qos,
                [this](const std_msgs::msg::Bool::ConstSharedPtr msg) {
                    this->gimbal_node_control_flag_2 = msg->data; }); // 조난자 구출 끝난 시점 플래그
...
private:
    ...
    rclcpp::Subscription<std_msgs::msg::Bool>::SharedPtr subscription_gimbal_node_control_flag;
    ...
    float fcc_position_N =0.0f; float fcc_position_E=0.0f; float fcc_position_D=0.0f;    
    geometry_msgs::msg::Point32 object_ned_msg={0.0,0.0,0.0};
    bool gimbal_node_control_flag_1 = false;
    bool gimbal_node_control_flag_2 = false;
    ...
    GimbalState current_state_ = FIX_CENTER;  // 처음엔 객체 중심 상태
    bool flag_active_ = false;                   // /gimbal_control_flag 토픽으로 세팅됨
    bool pitch_down_sent_ = false;   

    void control_loop() {
        switch (current_state_) {
            case FIX_CENTER:
                control_gimbal(FIX_CENTER);
                if (gimbal_node_control_flag_1==true) current_state_ = FOLLOW_OBJECT;
                break;
    
            case FOLLOW_OBJECT:
                control_gimbal(FOLLOW_OBJECT);
                if (gimbal_node_control_flag_2==true) current_state_ = FIX_DOWNWARD;
                break;
    
            case FIX_DOWNWARD:
                control_gimbal(FIX_DOWNWARD);
                break;
        }
    }
    ...
    void control_gimbal(GimbalState state) {
        int sockfd;
        int recv_len;
        ...
        if (state == FOLLOW_OBJECT && gimbal_node_control_flag_1) {
            std::vector<float> target = {object_ned_msg.x, object_ned_msg.y, object_ned_msg.z};
            std::vector<float> origin = {fcc_position_N, fcc_position_E, fcc_position_D};
            float dx = target[0] - origin[0];
            float dy = target[1] - origin[1];
            float dz = target[2] - origin[2];
    
            float pitch_deg = atan2(-dz, sqrt(dx*dx + dy*dy)) * (180.0 / M_PI);
            float yaw_deg = atan2(dy, dx) * (180.0 / M_PI);
            encode_attitude(yaw_deg, pitch_deg, yaw_hex, pitch_hex);
            
            if(gimbal_node_control_flag_2==true)
            {state =FIX_DOWNWARD;}
        } else if (state == FIX_CENTER) {
            yaw_hex[0] = 0x01; yaw_hex[1] = 0xd1;
            pitch_hex[0] = 0x12; pitch_hex[1] = 0x00;
    
        } else if (state == FIX_DOWNWARD) {
            yaw_hex[0] = 0x00; yaw_hex[1] = 0x00;
            pitch_hex[0] = 0xA6; pitch_hex[1] = 0xFF; // -90도
        }
