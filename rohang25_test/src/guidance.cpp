...
std::vector<double> precise_landing_guidance(double_t* param, double heading, geometry_msgs::msg::Point32 object_pos, std::vector<double> local_pos)
{
    ...
    if (mode == 0.0) {
        ...
        double ey = object_pos.y - gy;
        temp_cmd.x = -param[4] * ey;
        temp_cmd.y =  param[3] * ex;
        temp_cmd.z =  param[6];
        double hdg = heading * D2R;
        temp_ned.x = temp_cmd.x * cos(hdg) - temp_cmd.y * sin(hdg);
        temp_ned.y = temp_cmd.x * sin(hdg) + temp_cmd.y * cos(hdg);
        temp_ned.z = temp_cmd.z;
        cmd_ned    = vel_saturation(temp_ned, param[5]);
    }
    else if (mode == 1.0) {
        // NED-based control
        double gn = param[1];
